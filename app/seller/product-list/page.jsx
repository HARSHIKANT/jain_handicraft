...
const ProductList = () => {
  ...
  const handleToggleStock = async (product) => {
    try {
      const token = await getToken()
      const { data } = await axios.put('/api/product/stock', { productId: product._id }, {
        headers: { Authorization: `Bearer ${token}` }
      })
      if (data.success) {
        toast.success(data.message)
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <button
                        ...
                        className={`flex items-center gap-1 px-1.5 md:px-3 py-2 rounded-md text-xs ${product.outOfStock
                          ? 'bg-amber-500 text-white'
                          : 'bg-gray-200 text-gray-700'
                          }`}
                        title={product.outOfStock ? 'Mark as In Stock' : 'Mark as Out of Stock'}
                      >
                        <span className="hidden md:block">{product.outOfStock ? 'Out of Stock' : 'In Stock'}</span>
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
